...
COLD_FUNC void
globalcontext_destroy(GlobalContext *glb)
{
   free (glb);
}

Context *
globalcontext_get_process(GlobalContext *glb, int32_t process_id)
{
   Context *processes = GET_LIST_ENTRY(glb->processes_table,
                                       Context,
   ...
   do
   {
      ...
      p = GET_LIST_ENTRY(p->processes_table_head.next,
                         Context,
                         processes_table_head);
   } while (processes != p);
...
int32_t
globalcontext_get_new_process_id(GlobalContext *glb)
{
   glb->last_process_id++;
...
void
globalcontext_register_process(GlobalContext *glb,
                               int atom_index,
                               int local_process_id)
{
   ...
   linkedlist_append (&glb->registered_processes,
                      &registered_process->registered_processes_list_head);
}

int
globalcontext_get_registered_process(GlobalContext *glb, int atom_index)
{
   ...
   const struct RegisteredProcess *registered_processes = GET_LIST_ENTRY(glb->registered_processes,
                                                                         struct RegisteredProcess,
   ...
   do
   {
      ...
      p = GET_LIST_ENTRY(p->registered_processes_list_head.next,
                         struct RegisteredProcess,
                         registered_processes_list_head);
...
int
globalcontext_insert_atom(GlobalContext *glb, AtomString atom_string)
{
   struct AtomsHashTable *htable = glb->atoms_table;

   unsigned long atom_index = atomshashtable_get_value (htable,
                                                        atom_string,
                                                        ULONG_MAX);
   if (atom_index == ULONG_MAX)
   {
      atom_index = atomshashtable_get_count (htable);
      if (!atomshashtable_insert (htable, atom_string, atom_index))
      {
         return -1;
      }
      if (!valueshashtable_insert (glb->atoms_ids_table,
                                   atom_index,
                                   (unsigned long) atom_string))
      {
         return -1;
      }
   }
...
AtomString
globalcontext_atomstring_from_term(GlobalContext *glb, term t)
{
   if (!term_is_atom (t))
   {
      abort ();
   ...
   unsigned long ret        = valueshashtable_get_value (glb->atoms_ids_table,
                                                         atom_index,
                                                         ULONG_MAX);
   if (ret == ULONG_MAX)
   {
...
int
globalcontext_insert_module(GlobalContext *global,
                            Module *module,
                            AtomString module_name_atom)
{
   bool success = atomshashtable_insert (global->modules_table,
                                         module_name_atom,
                                         0 /* unused was -> TO_ATOMSHASHTABLE_VALUE (module)*/);
   if (success == false)
   {
      return -1; // TODO: Return real error
   }
...
void
globalcontext_insert_module_with_filename(GlobalContext *glb,
                                          Module *module,
                                          const char *filename)
{
...
Module *
globalcontext_get_module(GlobalContext *global, AtomString module_name_atom)
{
   Module *found_module = (Module *) atomshashtable_get_value (global
                                                                  ->modules_table,
                                                               module_name_atom,
                                                               (unsigned long) NULL);
   ...
   if (!found_module)
   {
      ...
      if (UNLIKELY(!loaded_module
                   || (globalcontext_insert_module (global,
                                                    loaded_module,
                                                    module_name_atom) < 0)))
      {
         return NULL;
